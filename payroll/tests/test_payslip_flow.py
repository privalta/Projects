...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        payslip_input = self.env["hr.payslip.input"].search([("payslip_id", "=", richard_payslip.id), ("code", "=", "SALEURO")])
        ...
        child_line = richard_payslip.dynamic_filtered_payslip_lines.filtered(lambda l: l.code == "NET_CHILD")
        self.assertEqual(len(child_line), 1, "Child line found when flag desactivated (default)")

        # Check parent line id value is correct
        parent_line = richard_payslip.dynamic_filtered_payslip_lines.filtered(lambda l: l.code == "NET")
        self.assertEqual(
        ...
        self.assertEqual(len(parent_line.parent_line_id), 0, "The parent line has no parent_line_id")
        ...
        child_line = richard_payslip.dynamic_filtered_payslip_lines.filtered(lambda l: l.code == "NET_CHILD")
        self.assertEqual(len(child_line), 0, "The child line is not found when flag activated")

        # Find the 'NET' payslip line and check that it adds up
        # salary + HRA + MA + SALE - PT
        work100 = richard_payslip.worked_days_line_ids.filtered(lambda x: x.code == "WORK100")
        ...
        payslip_employee = self.env["hr.payslip.employees"].create({"employee_ids": [(4, self.richard_emp.id)]})
        ...
        data, data_format = self.env.ref("payroll.action_report_payslip")._render_qweb_pdf(richard_payslip.ids)

        # I print the payslip details report
        data, data_format = self.env.ref("payroll.payslip_details_report")._render_qweb_pdf(richard_payslip.ids)
    ...
    def test_contract_qty(self):
        ...
        self.test_rule.amount_python_compute = "result = payroll.contracts and payroll.contracts.count or -1.0"
        ...
        self.assertEqual(line[0].amount, 1.0, "The calculated dictionary value 'contracts.qty' is 1")
    ...
    def test_compute_multiple_payslips(self):
        ...
        self.assertTrue(payslips[0].number, "The first payslip as been assigned a number")
        self.assertTrue(payslips[1].number, "The second payslip as been assigned a number")
    ...
    def test_get_contracts_singleton(self):
        ...
        self.apply_contract_cron()
        contracts = payslip._get_employee_contracts()
        self.assertEqual(len(contracts), 1, "There is one open contract for the employee")
        ...
        contracts = payslip._get_employee_contracts()
        self.assertEqual(len(contracts), 2, "There are two open contracts for the employee")
    ...
    def test_get_contracts_multiple(self):
        ...
        self.assertEqual(len(contracts), 3, "There are 3 open contracts in the payslips")
    ...
    def test_get_payslip_line_multi(self):
        ...
        self.assertIn(sally_key, lines_dict.keys(), "A line was created for Sally's contract")
        self.assertIn(richard_key, lines_dict.keys(), "A line was created for Richard's contract")
    ...
    def test_onchange_dates_singleton(self):
        ...
        payslip.worked_days_line_ids = [(0, 0, {"name": "A", "code": "A", "contract_id": self.sally.contract_id.id})]
    ...
    def test_onchange_dates_multi(self):
        ...
        payslips[1].worked_days_line_ids = [(0, 0, {"name": "B", "code": "B", "contract_id": self.sally.contract_id.id})]
    ...
    def test_employee_multiple_contracts(self):
        ...
        # 1st attempt
        contracts = payslip._get_employee_contracts()
        self.assertEqual(len(contracts), 2, "There are 2 open contracts in the payslips")
        # 2nd attempt (after setting payslip.contract_id) should yield same result
        payslip.contract_id = self.sally.contract_ids[0].id
        contracts = payslip._get_employee_contracts()
        self.assertEqual(len(contracts), 2, "There are still 2 open contracts in the payslips")
