...
class TestHrPayrollCancel(common.TransactionCase):
    def setUp(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("payroll.allow_cancel_payslips", True)
    ...
    def _create_payslip(self):
        ...
        date_to = datetime.now() + relativedelta.relativedelta(months=+2, day=1, days=-1)
        res = self.hr_payslip.get_payslip_vals(date_from, date_to, self.hr_employee_anita.id)
        vals = {
