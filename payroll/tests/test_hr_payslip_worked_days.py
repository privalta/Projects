...
class TestWorkedDays(TestPayslipBase):
    ...
    def test_worked_days_negative(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("payroll.leaves_positive", False)
        ...
        self.assertIn("TESTLV", worked_days_codes, "The leave is in the 'Worked Days' list")
        wdl_ids = richard_payslip.worked_days_line_ids.filtered(lambda x: x.code == "TESTLV")
    ...
    def test_leaves_positive(self):
        ...
        # Set system parameter
        self.env["ir.config_parameter"].sudo().set_param("payroll.leaves_positive", True)
        ...
        wdl_ids = richard_payslip.worked_days_line_ids.filtered(lambda x: x.code == "TESTLV")
        self.assertEqual(len(wdl_ids), 1, "There is only one line matching the leave")
        self.assertEqual(wdl_ids[0].number_of_days, 1.0, "The days worked value is a POSITIVE number")
        self.assertEqual(
            wdl_ids[0].number_of_hours,
            8.0,
