...
class HrPayslipLine(models.Model):
    ...
    child_ids = fields.One2many("hr.payslip.line", "parent_line_id", string="Child Payslip Lines")
    ...
    allow_edit_payslip_lines = fields.Boolean("Allow editing", compute="_compute_allow_edit_payslip_lines")

    def _compute_allow_edit_payslip_lines(self):
        self.allow_edit_payslip_lines = self.env["ir.config_parameter"].sudo().get_param("payroll.allow_edit_payslip_lines")

    @api.depends("parent_rule_id", "contract_id", "slip_id")
    def _compute_parent_line_id(self):
        for line in self:
            if line.parent_rule_id:
                parent_line = line.slip_id.line_ids.filtered(lambda l: l.salary_rule_id == line.parent_rule_id and l.contract_id == line.contract_id and l.slip_id == line.slip_id)
                if parent_line and len(parent_line) > 1:
                    raise UserError(_("Recursion error. Only one line should be parent of %s") % line.parent_rule_id.name)
                line.parent_line_id = parent_line[0].id if len(parent_line) == 1 else False
            else:
