...
from .base_browsable import BaseBrowsableObject, BrowsableObject, InputLine, Payslips, WorkedDays
...
class HrPayslip(models.Model):
    ...
    hide_invisible_lines = fields.Boolean(string="Show only lines that appear on payslip", default=False)
    compute_date = fields.Date("Compute Date")
    refunded_id = fields.Many2one("hr.payslip", string="Refunded Payslip", readonly=True)
    allow_cancel_payslips = fields.Boolean("Allow Canceling Payslips", compute="_compute_allow_cancel_payslips")
    prevent_compute_on_confirm = fields.Boolean("Prevent Compute on Confirm", compute="_compute_prevent_compute_on_confirm")

    def _compute_allow_cancel_payslips(self):
        self.allow_cancel_payslips = self.env["ir.config_parameter"].sudo().get_param("payroll.allow_cancel_payslips")

    def _compute_prevent_compute_on_confirm(self):
        self.prevent_compute_on_confirm = self.env["ir.config_parameter"].sudo().get_param("payroll.prevent_compute_on_confirm")
    ...
    def action_payslip_done(self):
        if not self.env.context.get("without_compute_sheet") and not self.prevent_compute_on_confirm:
            self.compute_sheet()
    ...
    def refund_sheet(self):
        for payslip in self:
            copied_payslip = payslip.copy({"credit_note": True, "name": _("Refund: %s") % payslip.name})
    ...
    def _compute_leave_days(self, contract, day_from, day_to):
        ...
        """
        leaves_positive = self.env["ir.config_parameter"].sudo().get_param("payroll.leaves_positive")
        leaves = {}
    ...
    def get_inputs(self, contracts, date_from, date_to):
        ...
        payslip_inputs = self.env["hr.salary.rule"].browse(sorted_rule_ids).mapped("input_ids")
    ...
    def _get_baselocaldict(self, contracts):
        ...
        input_lines_dict = {line.code: line for line in self.input_line_ids if line.code}
        localdict = {
            ...
            "payroll": BrowsableObject(self.employee_id.id, self.get_payroll_dict(contracts), self.env),
            ...
            "tools": BrowsableObject(self.employee_id.id, self._get_tools_dict(), self.env),
        }
        return localdict
    ...
    def _compute_payslip_line(self, rule, localdict, lines_dict):
        ...
        return self._get_lines_dict(rule, localdict, lines_dict, key, values, previous_amount)

    def _get_lines_dict(self, rule, localdict, lines_dict, key, values, previous_amount):
        ...
        localdict = self._sum_salary_rule_category(localdict, rule.category_id, total - previous_amount)
    ...
    def _get_payslip_lines(self, _contract_ids, payslip_id):
        _logger.warning("Use of _get_payslip_lines() is deprecated. " "Use get_lines_dict() instead.")
    ...
    def get_payslip_vals(self, date_from, date_to, employee_id=False, contract_id=False, struct_id=False):
    ...
    def _get_employee_contracts(self):
        ...
        for payslip in self:
            contracts |= payslip.employee_id._get_contracts(date_from=payslip.date_from, date_to=payslip.date_to)
    ...
    def onchange_struct_id(self):
        for payslip in self:
            ...
            input_line_ids = payslip.get_inputs(payslip._get_employee_contracts(), payslip.date_from, payslip.date_to)
    ...
    def onchange_dates(self):
        for payslip in self:
            ...
            worked_days_line_ids = payslip.get_worked_day_lines(payslip._get_employee_contracts(), payslip.date_from, payslip.date_to)
    ...
    def line_sum_where(self, field_name, value, rules, result_rules):
        ...
        """
        return sum([result_rules.dict[code].dict["total"] for code, rule in rules.dict.items() if getattr(rule, field_name) == value])
