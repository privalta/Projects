...
class HrSalaryRule(models.Model):
    ...
    def _compute_require_code_and_category(self):
        require = self.env["ir.config_parameter"].sudo().get_param("payroll.require_code_and_category")
    ...
    def _compute_rule_fix(self, localdict):
        try:
            ...
            raise UserError(_("Wrong quantity defined for salary rule %s (%s) for employee %s.") % (self.name, self.code, localdict["employee"].name))
    ...
    def _compute_rule_percentage(self, localdict):
        try:
            ...
            raise UserError(_("Wrong percentage base or quantity defined for salary " "rule %s (%s) for employee %s.") % (self.name, self.code, localdict["employee"].name))
...
"""
    ...
    def _satisfy_condition_range(self, localdict):
        try:
            ...
            raise UserError(_("Wrong range condition defined for salary rule %s (%s) for employee %s.") % (self.name, self.code, localdict["employee"].name))
